...
export const themeEffect = function () {
  ...
  if (pref === 'dark' || (!pref && window.matchMedia('(prefers-color-scheme: dark)').matches)) {
    ...
    document.head.querySelector('meta[name=theme-color]')?.setAttribute('content', theme.dark.bg)
    ...
    document.head.querySelector('meta[name=theme-color]')?.setAttribute('content', theme.light.bg)
    requestAnimationFrame(() => {
      document.documentElement.classList.remove('pause-transitions')
    })
    return 'light'
